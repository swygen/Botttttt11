...
GROUP_USERNAME = 'swygenbd'  # Telegram group username without @
...
async def prompt_join_group(message: types.Message):
    ...
    text = (
        f"বট ব্যবহারের আগে আপনাকে আমাদের অফিসিয়াল গ্রুপে যোগ দিতে হবে:\nhttps://t.me/{GROUP_USERNAME}\n\n"
...
async def set_language(callback_query: types.CallbackQuery):
    ...
    welcome_msg = (
        f"স্বাগতম, {user_full_name}! আপনার লোকেশন অনুযায়ী তথ্য পেতে নিচের অপশন থেকে বেছে নিন:"
        if lang == 'bn' else
        f"Welcome, {user_full_name}! Choose an option below to get location-based info:"
    )
...
async def process_callback(callback_query: types.CallbackQuery):
    ...
    prompt = "অনুগ্রহ করে 📍 লোকেশন পাঠান..." if lang == 'bn' else "Please send your 📍 location..."
...
async def handle_location(message: types.Message):
    ...
    if action == 'weather':
        ...
        result_text = "📍 *আপনার কাছাকাছি সেবা:*\n\n" if lang == 'bn' else "📍 *Nearby Services:*\n\n"
        if not places:
            sorry = "দুঃখিত, কিছুই পাওয়া যায়নি।" if lang == 'bn' else "Sorry, nothing found."
